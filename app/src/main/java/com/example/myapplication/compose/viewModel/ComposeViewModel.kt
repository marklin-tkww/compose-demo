package com.example.myapplication.compose.viewModel
...
class ComposeViewModel : ViewModel() {
    val functions = arrayListOf<FuncInfo>().apply {
        add(FuncInfo("布局显示", Screen.layout, Color(0xFF1BC3E0)))
        add(FuncInfo("文字展示", Screen.text, Color(0xFF078900)))
        add(FuncInfo("图片显示", Screen.image, Color(0xFFFFBE3B)))
        add(FuncInfo("列表显示", Screen.List.main, Color(0xFF00C29B)))
        add(FuncInfo("主题更改", Screen.theme, Color(0xFF6200EE)))
        add(FuncInfo("Canvas", Screen.canvas, Color(0xFF1D2E44)))
        add(FuncInfo("自定义布局", Screen.custom_layout, Color(0xFF3587EC)))
        add(FuncInfo("动画效果", Screen.animation, Color(0xFFDD26AF)))
        add(FuncInfo("手势", Screen.gesture, Color(0xFF00A1FF)))
    }
